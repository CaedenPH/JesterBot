...
class Games(commands.Cog):
    ...
    async def uno(self, ctx):
        ...
        try:
            received_msg = await self.client.wait_for('message', timeout=120.0, check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
            ...
            if received_msg.mentions:
                ...
                for k in received_msg.mentions:
                    ...
                    try:
                        emoji, user = await self.client.wait_for('reaction_add', timeout=120.0, check=lambda e, u: u in mentions) 
    ...
    async def on_dropdown(self, inter):
        if inter.message.id not in msglist:
            return
