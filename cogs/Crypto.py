...
class Cryptocurrency(commands.Cog):
    ...
    async def exchange(self, ctx, amount: int, exchangefrom, exchangeto):
        ...
        embed.add_field(
            name="Rate", value=response.json()["info"]["rate"], inline=False
        )

        data = response2.json()["symbols"]
        if exchangeto in data:
            embed.set_thumbnail(
                url=f"https://cryptoicons.org/api/icon/{exchangefrom}/200"
            )
        else:

            embed.set_thumbnail(
                url=f"https://cryptoicons.org/api/icon/{exchangeto}/200"
            )
    ...
    async def stocks(self, ctx: Context, stock=None):
        ...
        async with ctx.typing():
            ...
            if "fullTimeEmployees" in inf:
                embed.add_field(
                    name="Employees", value=f"{inf['fullTimeEmployees']}", inline=False
                )
            if "dividendRate":
                embed.add_field(
                    name="Employees", value=f"{inf['dividendRate']}", inline=False
                )
            if "country" in inf:
                embed.add_field(
                    name="Location",
                    value=f"{inf['country']}, {inf['state']}, {inf['city']}",
                    inline=False,
                )
            if "sharesShort" in inf:
                embed.add_field(
                    name="Shares", value=f"{inf['sharesShort']}", inline=False
                )
            if "fiftyTwoWeekLow" in inf:
                embed.add_field(
                    name="Value",
                    value=f"Market Cap: *${inf['marketCap']}*\nLow: *${inf['fiftyTwoWeekLow']}*\nHigh: *${inf['fiftyTwoWeekHigh']}*",
                    inline=False,
                )
            if "logo_url" in inf:
