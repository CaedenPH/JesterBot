...
class Crime(commands.Cog):
    ...
    async def wanted_criminals(self, ctx: Context) -> None:
        ...
        async with self.bot.client.get(url="https://api.fbi.gov/wanted/v1/list", params={'page': random.randint(0, 49)}) as resp:
...
{data}
    ...
    async def jailbase(self, ctx: Context, last_name:str, first_name:str="") -> None:
        async with self.bot.client.get(url=f"https://www.jailbase.com/api/1/search/?source_id=az-mcso&last_name={last_name}&first_name={first_name}") as resp:
            json = await resp.json()
...
{police}    
  ...
  [Factual information curated from official police data]```"""
    ...
    async def coordinates(self, ctx: Context, *, poste_code=None) -> None:
        async with self.bot.client.get(url=f"http://api.positionstack.com/v1/forward?access_key={self.bot.COORDS_KEY}&query={poste_code}") as resp:
            json = await resp.json()
