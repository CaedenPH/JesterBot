...
class ChatBot(commands.Cog):
    ...
    async def find_or_insert_channel(self, channel_id: int, **kwargs):
        ...
        await cursor.execute('Select * from chatbot where channel_id = ?', (channel_id,))
        ...
        if remove:
            await cursor.execute('Delete from chatbot where channel_id = ?', (channel_id,))        
    ...
    async def send_ai(self, channel: Context, message: str, avatar_url=None) -> None:
        ...
        embed = disnake.Embed(
            ...
            icon_url=avatar_url or disnake.Embed.Empty,
            name="Chatbot"
        )
        if not response:
    ...
    async def remove_ai(self, ctx: Context, channel: disnake.TextChannel) -> None:
        ...
        await channel.send(
            embed = disnake.Embed(
                description="Chatbot is removed!"
            )
        )
