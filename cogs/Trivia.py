...
class Trivia(commands.Cog):
    ...
    async def replace_token(self) -> None:
        async with self.bot.client.get(url="https://opentdb.com/api_token.php?command=request", verify_ssl=False) as resp:
            json = await resp.json()
            self.token = json['token']

    async def get_question(self):
        async with self.bot.client.get(url=f"https://opentdb.com/api.php?amount=1&token={self.token}", verify_ssl=False) as resp:
            json = await resp.json()
        ...
        while True:
            ...
            try:
                ...
                if not run:
                    ...
                    await ctx.em("```yaml\nNo one responded in time! If no one responds on the next round the game ends```", delete_after=60)
